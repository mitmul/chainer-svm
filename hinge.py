...
class Hinge(function.Function):
    ...
    def forward_cpu(self, inputs):
        ...
        for i in six.moves.range(x.shape[0]):
            for j in six.moves.range(x.shape[1]):
                self.bottom_diff[i, j] = max(0, 1 + self.bottom_diff[i, j])
        loss = 0
        if self.norm == 'L1':
            loss = numpy.sum(numpy.abs(self.bottom_diff)) / x.shape[0]
        elif self.norm == 'L2':
            loss = numpy.sum(self.bottom_diff ** 2) / x.shape[0]
        else:
    ...
    def forward_gpu(self, inputs):
        ...
        loss = 0
        if self.norm == 'L1':
            loss = cuda.cupy.sum(cuda.cupy.abs(self.bottom_diff)) / x.shape[0]
        elif self.norm == 'L2':
    ...
    def backward_cpu(self, inputs, grad_outputs):
        ...
        for i in six.moves.range(t.shape[0]):
            self.bottom_diff[i, t[i]] *= -1
        gx = numpy.zeros_like(self.bottom_diff)
        if self.norm == 'L1':
    ...
    def backward_gpu(self, inputs, grad_outputs):
        ...
        gx = xp.empty_like(self.bottom_diff)
        if self.norm == 'L1':
